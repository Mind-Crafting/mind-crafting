...
module.exports = class SlackService {
  ...
  static find(user) {
    ...
    const itemFrameId = player.mcData.blocksByName['item_frame'].id
    const blocks = player.findBlocks({ matching: itemFrameId });

    blocks.forEach(block => console.log(player.blockAt(block)));
