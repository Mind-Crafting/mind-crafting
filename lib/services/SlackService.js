...
module.exports = class SlackService {
  ...
  static play({ argument, user }) {
    ...
    try {
      ...
      const itemFrame = Bots.getGamemodeEntity(player, argument);
      const timeout = Bots.getDistance(player.entity, itemFrame) * this.oneBlock;
      ...
      player.on('spawn', () => {
        player
          ...
          .then(() => {
            ...
            setTimeout(() => player.setControlState('forward', timeout))
          })
      })
    }
