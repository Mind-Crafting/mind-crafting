...
module.exports = class Library {
  ...
  botFindGamemode(player, gamemodeCommand) {
    player.once('spawn', () => {
      ...
      const itemFrame = this.getGamemodeEntity(player, gamemodeCommand);
      const timeout = (this.oneBlock + 1) * this.getDistance(player.entity, itemFrame);
      ...
      setTimeout(() => player.lookAt(itemFrame.position, false, () => {
        ...
        slackBot
          ...
          .postMessage({
            channel: 'general',
            text: `Your viewer link is ${tihs.awsLink}?port=${player.viewerPort}`
          });
      }), 1000);
    })
  }
